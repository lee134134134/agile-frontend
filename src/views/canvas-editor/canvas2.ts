...
import diagramPlugin from '../../components/diagram'
...
export default function Init(content: any) {
    ...
    const convertSvgToPng = (svgBase64, width, height) => {
        return new Promise((resolve) => {
            const img = new Image();
            img.onload = () => {
                const canvas = document.createElement('canvas');
                // 建议使用设备像素比以保证在编辑器内不模糊
                const dpr = window.devicePixelRatio || 1;
                canvas.width = (width || img.width) * dpr;
                canvas.height = (height || img.height) * dpr;
    ...
    //流程图
    const lctDom = document.querySelector<HTMLDivElement>('.menu-item__lct')!
    lctDom.onclick = function () {
        instance.command.executeLoadDiagram({
            lang: 'zh',
            data: '',
            onDestroy: async (message) => {
                const {bounds, data} = message;
                instance.command.executeInsertElementList([
                    {
                        type: ElementType.IMAGE,
                        width: bounds.width,
