    ...
    <el-dialog v-model="detailShow" :show-close="false" width="604px" @close="handleClose" :close-on-click-modal="false" :close-on-press-escape="false" class="edit-dia">
        <template #header="{ close }">
            <div class="dia-header">
                <div class="dia-header-title">
                    ...
                    <div class="title-text">{{ type }}角色</div>
                </div>
        ...
        <div class="dia-main">
            <el-form ref="detailFromRef" :model="detailFrom" :rules="detailRule" class="dia-main-content"
                ...
                <el-form-item label="角色简称" prop="name">
                    <el-input v-model="detailFrom.name" placeholder="请输入角色简称" maxlength="20" clearable></el-input>
                </el-form-item>
                <el-form-item label="角色名称" prop="displayName">
                    <el-input v-model="detailFrom.displayName" placeholder="请输入角色名称" maxlength="30" clearable></el-input>
                </el-form-item>

                <el-form-item label="描述" class="no-rquired">
                    <el-input v-model="detailFrom.description" placeholder="请输入角色描述" autosize type="textarea"
...
const props = defineProps(['type', 'isShow', 'roleInfo']);
...
const checkName = (rule, value, callback) => {
    if (value == '') {
        return callback(new Error('请输入角色简称！'))
    } else if (!isValidEChar(value)) {
        return callback(new Error('您输入的角色简称格式不正确'))
    } else {
        return callback()
    }
}
const detailRule = {
    ...
    displayName: [
        ...
        { min: 1, max: 30, message: '长度在 1 到 30 个字符', trigger: 'submit' }
    ]
}
        ...
        detailFromRef.value.validate((valid) => {
            if (valid) {
                ...
                if (props.type == '新增') {
                    ...
                    detailFrom.id = props.roleInfo.id
...
onBeforeMount(() => {
    if (props.type == '编辑') {
        detailFrom.displayName = props.roleInfo.displayName
        detailFrom.name = props.roleInfo.name
        detailFrom.description = props.roleInfo.description
    }
})
