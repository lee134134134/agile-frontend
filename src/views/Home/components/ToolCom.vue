<template>
  <div class="tool-content">
    ...
    <div v-if="activeTab == 1" class="menu-content-1 jus-start-center">
      <div v-for="item in agileMenu" :key="item.path" class="menu-items jus-bet-center" @click="goPage(item.path)">
        <div class="jus-start-center">
          <!-- <div class="title-icon"></div> -->
          <img alt="" src="@/assets/images/home/tool.png" style="width: 60px;margin-right: 12px;"/>
    ...
    <!-- <div class="menu-content jus-start-center" v-if="activeTab == 2">
        <div class="menu-items jus-bet-center" v-for="item in systemMenu" :key="item.path" @click="goPage(item.path)">
            <div class="jus-start-center">
                <div class="title-icon"></div>
                <div class="item-title">{{ item.meta.title }}</div>
            </div>
...
.tool-content {
  ...
  .menu-content-1 {
    ...
    .menu-items {
      ...
      background: linear-gradient(180deg, #DFECFF 0%, rgba(233, 236, 255, 0.3) 100%);
      cursor: pointer;
      flex-shrink: 0;

      &:hover{
        border: 1px solid #c3d7f3;
        box-shadow: 0 4px 12px 0 rgba(29,124,255,0.3);
      }
