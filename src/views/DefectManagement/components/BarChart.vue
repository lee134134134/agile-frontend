...
function generateOption(data) {
  return {
    ...
    xAxis: {
      type: 'category',
      axisLabel: {rotate: props.rotateDeg ? 45 : 0, margin: 20,fontSize: 12, width:150,overflow:'truncate',ellipsis: '...'},
    ...
    series: data.data.reduce((series, value, index) => {
      series.push({
        name: data.xaxis[index],
        type: 'bar',
        data: data.data.map((v, i) => i == index ? v : ''),
        itemStyle: {color: colorList[index % colorList.length]},
        label: {show: true, position: 'top'},
        barWidth: data.data[index] == 0 ? 0 : 30,
        barGap: '-100%',
        barGategoryGap: '20%',
      })
      return series
