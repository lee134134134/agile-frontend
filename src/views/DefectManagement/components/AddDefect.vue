  ...
  <el-dialog v-model="addShow" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false" class="edit-dia"
    ...
    <div class="dia-main">
      <el-form ref="detailFormRef" :model="detailForm" :rules="type == '查看' ? null : detailRule"
        ...
        <el-row :gutter="24">
          <el-col :span="24">
            <el-form-item label="缺陷标题" prop="defectTitle">
              <el-input v-model="detailForm.defectTitle" :disabled="isDisabled" clearable maxlength="50"
        ...
        <el-row :gutter="24">
          <el-col :span="12">
            <el-form-item label="所属项目" prop="projectId">
              <el-select v-model="detailForm.projectId" :disabled="isDisabled" clearable placeholder="请选择"
                         @change="handleChangeProject">
                <el-option v-for="item in dicStore.curUserProjectList" :key="item.projectId" :label="item.name"
          ...
          <el-col :span="12">
            <el-form-item label="所属迭代" prop="sprintId">
              <el-select v-model="detailForm.sprintId" :disabled="isDisabled" clearable placeholder="请选择"
                         style="width: 100%;" @change="handleChangeSprint">
        ...
        <el-row :gutter="24">
          <el-col :span="24">
            <el-form-item label="用户故事" prop="storyId">
              <el-select v-model="detailForm.storyId" :disabled="isDisabled" clearable placeholder="请选择">
        ...
        <el-row :gutter="24">
          <el-col :span="24">
            <el-form-item v-show="!(isDisabled && detailForm.fileInfo == 0)" class="attach-form-item" label="上传附件"
                          prop="fileInfo">
              ...
              >
                <div :class="{'opt-btn-disabled': isDisabled}" class="opt-btn">上传附件</div>
              </el-upload>
              <!-- 上传的附件列表 -->
              <div class="fileList-area">
                <div v-for="item in detailForm.fileInfo" :key="item.fileName" class="file-item">
                  <img alt="" class="doc-icon" src="../../../assets/images/docIcon.png">
                  <div class="doc-name">
                    <div class="name">{{ item.originName }}</div>
                    <div v-if="item.size" class="fileSize">{{ (Number(item.size) / 1024 / 1024).toFixed(2) }}M</div>
                  </div>
                  <div v-if="!isDisabled" :class="{'delete-box-disabled': isDisabled}" class="delete-box jus-bet-center"
        ...
        <el-row :gutter="24">
          <el-col :span="12">
            <el-form-item label="优先级" prop="defectPriority">
              <el-select v-model="detailForm.defectPriority" :disabled="isDisabled" clearable placeholder="请选择">
          ...
          <el-col :span="12">
            <el-form-item label="缺陷类型" prop="defectType">
              <el-select v-model="detailForm.defectType" :disabled="isDisabled" clearable placeholder="请选择">
                <el-option v-for="item in dicStore.defectTypeList" :key="item.value" :label="item.name"
                           :value="item.value"/>
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="24">
          <el-col :span="24">
            <el-form-item label="严重程度" prop="severity">
              <el-select v-model="detailForm.severity" :disabled="isDisabled" clearable placeholder="请选择">
                <el-option v-for="item in dicStore.severityList" :key="item.value" :label="item.name"
                           :value="item.value"/>
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="24">
          ...
          <el-col :span="12">
            <el-form-item label="缺陷环境" prop="environment">
              <el-select v-model="detailForm.environment" :disabled="isDisabled" clearable placeholder="请选择">
                <el-option v-for="item in dicStore.defectEnvList" :key="item.value" :label="item.name"
                           :value="item.value"/>
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="24">
          <el-col :span="12">
            <el-form-item label="处理人" prop="headlerAccount">
              <el-select v-model="detailForm.headlerAccount" :disabled="isDisabled" clearable placeholder="请选择">
          ...
          <el-col :span="12">
            <el-form-item label="负责人" prop="leaderAccount">
              <el-select v-model="detailForm.leaderAccount" :disabled="isDisabled" clearable placeholder="请选择">
                <el-option v-for="item in dicStore.allUserList" :key="item.username" :label="item.nickname"
                           :value="item.username"/>
              </el-select>
            </el-form-item>
        ...
        <el-row :gutter="24">
          <el-col :span="12">
            <el-form-item label="缺陷状态" prop="defectStatus">
              <el-select v-model="detailForm.defectStatus" :disabled="isDisabled" clearable placeholder="请选择">
          ...
          <el-col :span="12">
            <el-form-item label="处理结果" prop="result">
              <el-select v-model="detailForm.result" :disabled="isDisabled" clearable placeholder="请选择">
                <el-option v-for="item in dicStore.handleResultList" :key="item.value" :label="item.name"
                           :value="item.value"/>
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="24">
          <el-col :span="24">
            <el-form-item label="开发团队" prop="teamName">
              <el-input v-model="detailForm.teamName" :disabled="isDisabled" clearable maxlength="20"
                        placeholder="请输入" show-word-limit>
                <!-- <template #suffix>
                  最多20字符
                </template> -->
        ...
        <div v-if="type != '新增' && detailForm.relationList && detailForm.relationList.length > 0"
             class="module-title mt20">已关联信息
        </div>
        <div v-for="(item, index) in detailForm.relationList"
             v-if="type != '新增' && detailForm.relationList && detailForm.relationList.length > 0" :key="index" class="link-item jus-bet-start">
          ...
          <div class="link-item-info">
            ...
            <!-- <div class="">解除关联</div> -->
            <el-button :class="{ 'disabled-btn': type == '查看' || item.defectStatus == 'suspend' }" :disabled="type == '查看' || item.defectStatus == 'suspend'"
                       class="btn-text-primary text-btn"
                       type="text"
                       @click="handleUnbindFn(item.defectId)">解除关联
            </el-button>
          </div>
        </div>
...
import {getIterUserStoryList, getUserProjectIterationList} from '@/api/api';
import {downloadSingleAttach, uploadAttach} from '@/api/defectApi';
...
const handleCreated = (editor) => {
  console.log('当前工具栏配置', editor.getConfig())
