<template>
    <div class="sql-editor cm-s-robot">
      <textarea class="code-mirror" :id="editorId"></textarea>
    </div>
  </template>
  ...
  const props = defineProps(['defaultEditorText', 'editorId'])
  ...
  const init = () => {
    codemirror.value = CodeMirror.fromTextArea(document.getElementById(`${props.editorId}`), {
            mode: 'text/x-robotframework', // 指定 SQL 模式
            theme: 'robot-framework', // 对应引入的主题
            lineNumbers: true, // 显示行号
            autoCloseBrackets: true, // 自动补全括号
    ...
    codemirror.value.setOption("readOnly", true)
  }
  ...
  defineExpose({ getEditorValue,  handleRefreshEditor, handleCoverEditor })
