...
class htmlToPdf {
  constructor(ele, pdfFileName, splitClassName,download = true, breakClassName = "break_page", sucCallback = () => { }, scale = 2) {
    this.ele = ele;
  ...
  async getPDF(resolve) {
    ...
    html2canvas(ele, {
      ...
      let imgWidth = 592.28 - 10 // -10为了页面有右边距
      ...
      if (leftHeight < this.pageHeight) { // 在pdf.addImage(pageData, 'JPEG', 左，上，宽度，高度)设置在pdf中显示；
        ...
        while (leftHeight > 0) {
          pdf.addImage(pageData, "JPEG", 6, position+15, imgWidth - 12, imgHeight)
    ...
    childNodes.forEach((childDom, index) => {
      if (this.hasClass(childDom, this.splitClassName)) {
        ...
        if (this.pageNum < currentPage) {
          ...
          newNode.style.height = (this.pageHeight * (this.pageNum - 1) - offset2Ele + 50) + 'px'; // +30为了在换下一页时有顶部的边距
